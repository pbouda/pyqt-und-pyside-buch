...
als Parameter übergeben werden. Die minimale Größe des Stretchelements
ist immer null, so dass es gegebenenfalls Platz für die Darstellung
aller anderen Widgets macht.


\ospsubsection{Elemente im Raster}{Elemente im Raster}{sec:gridlayouts}
...
das Widget in Breite und Höhe einnimmt (standardmäßig genau eine
Spalte und Zeile).

Um die drei Elemente in einem Rasterlayout anzuordnen, ändern Sie die
Methode \ospcmd{createLayout()} folgendermaßen ab:
...
Parameter aufnehmen: Zeile und Spalte für das Widget sowie die Größe
in Rastereinheiten in Höhe und Breite, also wie viele Zeilen und
Spalten das Widget umspannen soll. Die Größe des Grid Layouts wird
...
eine Zeile doppelt so groß wie die anderen definiert werden. Dazu
...
zu definieren ist. Für die Berechnung des Layouts spielt außerdem noch
...
schließlich auf dem Bildschirm erscheinen. Es gilt außerdem wieder,
dass die Größe des Hauptfensters letztlich auch Einfluss auf das
...
\ospsubsection{Verschachteln von Layouts}{Verschachteln von Layouts}{sec:schachtellayouts}
...
Rasterlayouts ausweichen; aus Erfahrung lässt sich das Aussehen einer
Anwendung mit verschachtelten Layouts aber wesentlich genauer
definieren. Häufig sind also horizontale und vertikale Layouts das
...
\ospsubsection{Stapellayouts}{Stapellayouts}{sec:stapellayouts}
...
Hauptfenster über ein zentrales Widget zugewiesen. Standardmäßig ist
...
\ospsubsection{Tab-Widget}{Tab-Widget}{sec:tabwidget}
...
Tab-Widgets sieht dann folgendermaßen aus:
...
Schließlich ist es natürlich sinnvoll, das Tab-Widget schon in der
Methode \ospcmd{createComponents()} zu erzeugen -- schließlich handelt
...
\ospsection{Signale und Slots}{Signale und Slots}{sec:signalslot}
...
PyQt und PySide stellen außerdem je einen Dekorator zur Verfügung, mit
...
Unter PySide heißt der entsprechende Dekorator \ospcmd{@Slot()}:
...
Signale eingegangen. Häufig braucht man eigene Signale erst aber einer
bestimmten Ausbaustufe von Anwendungen, in vielen Fällen reichen für
GUIs aber auch die Signale und Slots der Qt-Widgets aus, um die
...
\ospcmd{QAction} heißt \ospcmd{triggered()}. Hier der komplette Code
...
\ospsubsection{Signale und Slots für das Stapellayout}{Signale und Slots für das Stapellayout}{sec:sigslotstapel}
...
\ospfigure{0.8}{images/stapellayoutmitcombobox}{Ein Stapellayout mit Combobox}{fig:stapellayoutmitcombobox}
...
an. Die Anwendung benötigt außerdem ein neues, zusätzliches Layout,
schließlich können wir die Combobox nicht innerhalb des Stapellayouts
...
gesamte Methode \ospcmd{createLayout()} sieht dann folgendermaßen aus:
...
\ospsection{Dialoge}{Dialoge}{sec:dialoge}
...
erzwingen und erst nach Schließen des Fensters mit der Anwendung
...
Suchfenster zu schließen, weiter an dem Dokument arbeiten kann.  Heute
würde man allerdings stattdessen eher eine (an- und abdockbare)
Seitenleiste verwenden. Allgemeinen sind nicht-modale Dialoge aus der
...
Um Code und Inhalt der Dialoge verständlicher zu gestalten, werden die
...
Dialogen immer alle Strings mit \ospcmd{self.tr()} umschließen.

\ospsubsection{Dialoge für Nachrichten und Fragen}{Dialoge für Nachrichten und Fragen}{sec:messagebox}
...
Benutzer schließt das Fenster mit einem Klick auf einen Button. Bei
...
zuerst unsere Hauptfenster-Klasse als Elternobjekt, dann den Text.
Erweitern Sie dazu die Beispielanwendung um folgende
\ospcmd{zeigeUeberDialog()}-Methode, die im vorherigen Abschnitt als
...
als Rich-Text, d.\,h. mit Formatierungen, im Dialog dar.  Abbildung
\ospfigref{fig:messageboxinformation} zeigt das Informationsfenster
unter Windows. 


\ospfigure{0.5}{images/messageboxinformation}{Nachricht in einem Dialog}{fig:messageboxinformation}

Das Icon im Dialog wird von Qt automatisch gewählt, per
\ospmenu{OK} schließt der Benutzer das Fenster. Für unseren Fall des
...
Außerdem liefert die Methode nun einen Statuscode zurück, der anzeigt,
...
entsprechenden Enumerator-Werten übergeben werden.

\ospfigure{0.5}{images/messageboxquestion}{Frage in einem Dialog}{fig:messageboxquestion}
...
schon vorgestellten, die Dialoge zeigen anschließend entsprechende
...
Kritische Fehler lassen sich dem Benutzer folgendermaßen mitteilen:
...
anschließend der Rückgabewert des Fensters weiter verarbeitet.
...
\ospmenu{Schließen}-Knopf wird (unter Windows) in jedem Fall unten
rechts angezeigt, weil dies die Standardposition eines solchen Buttons
unter Windows ist.


\ospsubsection{Datei- und Verzeichnisdialoge}{Datei- und Verzeichnisdialoge}{sec:filedialogs}
...
in dem der Benutzer durch das Dateisystem browsen kann, um schließlich
...
\begin{ospsimplelisting}
QtGui.QDesktopServices.storageLocation(QtGui.QDesktopServices.PicturesLocation)\end{ospsimplelisting}
 
liefert den Pfad zum Bilder-Ordner und kann so als dritter
Parameter an \ospcmd{getOpenFileName()} übergeben werden.
...
\begin{ospdeflist}
  ...
  für benutzerspezifische temporäre Dateien
\end{ospdeflist}

\index{QFileDialog!getOpenFileNames()}%
\index{QStringList}%
\index{QList}%
Möchte man mehrere Dateien auf einmal vom Benutzer abfragen,  ruft
...
lassen sich in Python sehr einfach mit den Standarditeratoren wie
\ospcmd{for~... in~...} durchlaufen. Der Einfachheit halber geben wir
...
\index{Dialoge!Verzeichnisdialoge}%
Schließlich besteht die Möglichkeit, über
...
\ospsubsection{Eingaben abfragen}{Eingaben abfragen}{sec:inputdialogs}
...
\ospsubsubsection{Zahlen abfragen}{Zahlen abfragen}{sec:inputdialognumbers}
...
der Position immer gemäß dem Look\&{}Feel des Systems dar.
...
Für die Abfrage von reellen Zahlen, also Komma-Werten, existiert die
Methode \ospcmd{getDouble()}. Der siebte Parameter gibt statt der
Schrittweite nun die Zahl der Nachkommstellen an:
...
\ospsubsubsection{Aus Werten auswählen}{Aus Werten auswählen}{sec:getitemdialog}
...
einen Python-Unicode-String zu wandeln, wenn Sie mit PyQt arbeiten.
Der vorletzte Parameter gibt an, ob der Benutzer einen eigenen Text
statt der vorgegebenen eingeben kann. Standardmäßig ist dieser Wert
...
\ospsubsubsection{Einen String einlesen}{Einen String einlesen}{sec:inputdialogstring}
...
werden, oder auch \ospcmd{QLineEdit.NoEcho}, wenn die Eingabe gar
nicht sichtbar sein soll. Der darauf folgende Parameter übergibt dem
...
\ospsubsection{Eigene Dialoge erstellen}{Eigene Dialoge erstellen}{sec:customdialogs}
...
\item für einfach gehaltene Dialoge wird ein \ospcmd{QDialog} erzeugt,
  anschließend alle gewünschten GUI-Elemente und Layouts, die dem
  Dialog zugewiesen werden;

\item oder es wird, für komplexere Dialog, eine eigene Klasse von
...
\ospsubsubsection{Dialoge direkt mit QDialog erstellen}{Dialoge direkt mit QDialog erstellen}{sec:customdialogbyobject}
...
Text-Label und einem Button, der das Fenster schließt. Der Dialog soll
außerdem ein horizontales Layout bekommen, so dass beide Elemente
...
erst nach Schließen des Dialogs mit dem Hauptfenster weiter arbeiten
...
Schließen des Dialogs auch der Speicherplatz all seiner Elemente
...
\ospcmd{setWindowTitle}. Und schließlich sorgen wir mit einer
...
Mit \ospcmd{exec\_()} wird der Dialog schließlich angezeigt. Der
...
schließen zu müssen. Abbildung \ospfigref{fig:customdialogwindowsubuntu2}
...
Warum heißt der Slot aber nun \ospcmd{accept()} und nicht
...
Anschließend lässt sich der Rückgabewert des Dialogs auswerten:
...
Eingabefeld ließe sich damit folgendermaßen realisieren:
...
\ospsubsubsection{Dialoge als Ableitung von QDialog}{Dialoge als Ableitung von QDialog}{sec:customdialogsbyclass}
...
außerdem den Vorteil, dass analog zu unserem Programmgerüst des
...
class MeinDialog(QtGui.QDialog):
    ...
    def __init__(self, *args):
        ...
        button = QtGui.QPushButton(u"Schließe mich")
        layoutDialog = QtGui.QHBoxLayout()
        layoutDialog.addWidget(label)
        layoutDialog.addWidget(button)
...
Dialog schließt. In der Hauptfenster-Klasse können Sie diesen Dialog
...
Methoden. Außerdem lassen sich nun Attribute in der Klasse verwalten,
die Sie später vom aufrufenden Objekt aus auslesen können. Dazu können
Sie die Klasse \ospcmd{MeinDialog} folgendermaßen ändern:
...
class MeinDialog(QtGui.QDialog):
    ...
    def createComponents(self):
        ...
        self.button = QtGui.QPushButton(u"Schließe mich")
...
Dialog dann folgendermaßen erzeugt und der Wert aus dem Eingabefeld
ausgelesen werden:
...
\ospsection{Event-Handler}{Event-Handler}{sec:eventhandler}
...
Ereignisse, wie z.\,B. das Schließen des Dialogs oder Fensters,
...
\index{QObject!event()}%
Qt schickt Ereignisse, wie beispielsweise eine Größenänderung des
Darstellungsbereiches bei Änderungen der Hauptfenstergröße, über die
...
(im Falle der Größenänderung \ospcmd{resizeEvent()}), den wir einfach
...
Programmablauf verzweigen.  Schließlich unterstützt Qt das Konzept des
...
\ospsubsection{Spezielle Event-Handler}{Spezielle Event-Handler}{sec:spezielleeventhandler}
...
geschickt werden. Standardmäßig werden alle diese Ereignisse, wie
...
Für bestimmte Vorgänge gibt es jedoch kein eigenes Signal. Außerdem
...
\index{Events!Größenänderung des Widgets}%
\index{QWidget!closeEvent()}%
\index{QWidget!changeEvent()}%
\index{QWidget!resizeEvent()}%

\begin{osplist}
\item das Speichern von Einstellungen beim Schließen des
...
\item die Vorbereitung für einen Layout-Wechsel bei einer
  Größenänderung des Hauptfensters, per \ospcmd{resizeEvent()}.
\end{osplist}
...
Größe des Hauptfensters ändern, werden die neuen Werte für Breite und
...
Breite größer als die Höhe, definieren Sie für das zentrale Layout
...
Dialoge auf Größenänderungen überprüfen können.
...
\ospcmd{closeEvent()}, der das Ereignis beim Schließen eines Fensters
...
oder \ospcmd{int} umgewandelt werden, anschließend erfolgt für
\ospcmd{QStrings} die schon bekannte Konversion in einen Python-Typ
...
\ospcmd{closeEvent()}-Methode in \ospcmd{mainwindow.cpp}:
...
Einstellungen und Schließen beispielsweise ein Dialog eingeblendet
werden, ob der Benutzer die Einstellungen speichern oder nicht
speichern oder das Schließen der Anwendung abbrechen und zum
Hauptfenster zurückkehren will. Falls der Benutzer auf
\ospmenu{Abbrechen} klickt, kann man mit dem Aufruf von
\ospcmd{event.ignore()} das Schließen des Fensters verhindern. 

Wir wandeln außerdem an dieser Stelle den String aus dem Eingabefeld
...
in einen Python-Unicode-String um, auch wenn wir ihn schließlich nur
...
könnte nun folgendermaßen aussehen -- der Einfachheit halber setzen
...
\ospsubsection{Der allgemeine Event-Handler}{Der allgemeine Event-Handler}{sec:geneventhandler}

\index{Events!alle Events abfangen}%
\index{QEvent!type()}%
Neben den spezialisierten Event-Handlern für das Schließen von
Fenstern, Größenänderungen usw. gibt es mit \ospcmd{event()} einen
allgemeinen Event-Handler, den das Qt-Framework bei allen auftretenden
Ereignissen aufruft. Auch dieser stellt einen Ansatzpunkt für die
Verarbeitung von Ereignissen dar. Wie der Handler
...
ein Ereignis verarbeitet haben. Außerdem sollte man nicht vergessen,
...
verarbeitet. Außerdem steht dabei auch immer das spezialisierte
Event-Objekt zur Verfügung, so dass der Typ des Ereignisses nicht mehr
abgefragt werden muss.


\ospsubsection{Event-Filter}{Event-Filter}{sec:eventfilter}
...
keine besonderen Voraussetzungen erfüllen muss, außer dass seine
...
\ospcmd{QWidgets} an, für die der Entwickler keine eigene Klasse
...
den vorangegangen Abschnitten dargestellt. Wenn man außer den
...
Typ \ospcmd{QEvent.KeyPress} ist, und wandeln dann zu weiteren
Verarbeitung das allgemeine \ospcmd{QEvent}-Objekt in ein Objekt vom
Typ \ospcmd{QKeyEvent}. Über dieses erhalten wir per
\ospcmd{key()}-Methode den Key-Code der gedrückten Taste. Um
herauszufinden, welche Taste genau gedrückt wurde, können wir den
empfangenen Key-Code mit einem Wert des Enumerators \ospcmd{Qt.Key}
vergleichen. Der Event-Filter in der Hauptfenster-Klasse sieht dann
folgendermaßen aus:
...
Schließlich ist der Event-Handler für das Eingabefeld
...
und man spart sich den Aufwand, eine neue Klasse erstellen zu müssen.


\ospsection{Qt-Klassen und -Objekte}{Qt-Klassen und -Objekte}{sec:qtclassesandobjects}
...
\ospsubsection{QObject, QWidget und abgeleitete Klassen}{QObject, QWidget und abgeleitete Klassen}{sec:klassenhierarchie}

\index{Klassenhierarchie}%
\index{QObject}%
\index{QWidget}%
\index{QLayout}%
Die GUI-Klassen von Qt -- und einige andere hier nicht näher
Vorgestellte -- sind alle von der Basisklasse \ospcmd{QObject}
abgeleitet. Als Faustregel lässt sich außerdem festhalten, dass alle
...
solche, die sich in der Anwendung direkt benutzen lassen. Abstrakte
hingegen stellen wie \ospcmd{QLayout} und \ospcmd{QWidget}
Basisklassen für eine Menge an (konkreten oder abstrakten)
Unterklassen dar. 
...
\ospfigure{0.9}{images/qtklassenhierarchie}{Qt-Klassenhierarchie für QObject als Basisklasse}{fig:qtklassenhierarchie}


\ospsubsection{QString, QVariant, Container-Klassen und andere}{QString, QVariant, Container-Klassen und andere}{sec:qstringandothers}
...
da die entsprechenden Python-Klassen eine umfangreiche Schnittstelle
bieten und wesentlich allgemeiner in Python verwendet werden können
...
\ospsubsection{Objekthierarchie und Speicherverwaltung}{Objekthierarchie und Speicherverwaltung}{sec:objekthierarchie}
...
dass das Objekt ein Elternobjekt zugewiesen bekommt, häufig passiert
das schon bei der Erzeugung durch Übergabe eines
\ospcmd{parent}-Arguments an den Konstruktor.

\index{QMainWindow!setCentralWidget()}%
In unseren Beispielen sah das bisher so aus: In der
Hauptfenster-Klasse erzeugen wir ein zentrales Widgets, dem wir ein
Basis-Layout für das Hauptfenster zuweisen. Per
...
\ref{sec:hauptfenster} ergibt sich damit eine Hierarchie, wie in
Abbildung \ospfigref{fig:qtobjekthierarchie} dargestellt. Der
...
\ref{sec:hauptfenster} ließe sich damit auch folgendermaßen erzeugen:
...
kümmert. Im Gegensatz zu C++ werden dadurch aber neu referenzierte
...
Dies unter der Annahme, dass \ospcmd{self} ein Objekt einer von
...
diese erst wieder freigeben, sobald es selbst gelöscht wird.
Grundsätzlich sollte man solche Objekte aber als je eigenes Attribut
im Elternobjekt erzeugen, so dass man später über den Attributnamen
...
\ospsection{Übersicht über die Qt-Dokumentation}{Übersicht über die Qt-Dokumentation}{sec:qtdocumentation}

\index{Dokumentation}%
\index{Online-Dokumentation}%
Die Dokumentation des Qt-Frameworks lässt sich durchaus als exzellent
beschreiben. Für jeden Entwickler ist sie daher erste Anlaufstelle bei
Fragen und Problemen mit allen Qt-Klassen. Die Klassen werden im
Detail beschrieben, für viele gibt es außerdem einen Einführungstext,
der ihren Einsatz an Beispielen erklärt.  Außerdem enthält die
...
verfügbar.\ospfootnote{fn:qtdoc}{\ospurl{http://doc.qt.nokia.com/4.7/index.html}}
...
sich das Wissen um schnelle Nachschlagemöglichkeiten in einer erhöhten
Produktivität bei der Soft\-ware\-entwicklung niederschlagen.
