...
weit wie möglich voneinander zu trennen, um in grö\ss{}eren Projekten die
...
befindliche Tools von Qt Quick.  Anschlie\ss{}end gehen wir näher auf die
Programmierung mit QML ein, bevor es um die Anschlussmöglichkeiten von
QML-GUIs an Python-Code unter PyQt und PySide geht.


\section{Überblick über Qt Quick}
\label{sec:qtquickoverview}
...
Programmieren mit QML. Anschlie\ss{}end stellen wir mit dem QML Viewer
...
schon einmal testen und schnell Prototypen entwickeln. Schlie\ss{}lich
...
gro\ss{}e Baustelle, so dass wir im Weiteren auf die Beschreibung und den
Einsatz des Qt Designers für die QML-Code-Erstellung verzichten.


\subsection{Deklaratives Programmieren mit QML}
\label{sec:declarativeprogramming}
...
verbreitet hat und sich rasant weiterentwickelt hat. Au\ss{}erdem hat sich
...
QML-Objekten per Javascript kein grö\ss{}erer Lernprozess erwartet. Im
...
Darüber hinaus wurden im Beispiel lediglich Grö\ss{}e und Farbe des
Rechtecks deklariert. Au\ss{}erdem kann jedem QML-Element eine ID
...
darauf klickt, kann der bisherige Code einfach folgenderma\ss{}en
erweitert werden:
...
kann. Der \ospcmd{onClicked}-Handler beschreibt schlie\ss{}lich, was beim
...
meisten Handlern werden au\ss{}erdem die Daten zum Ereignis als Parameter
...
\section{QML-Anwendungen}
\label{sec:pureqmlapplications}
...
\subsection{Ein erster View in QML}
\label{sec:firstqmlview}
...
mit den Werten für die Grö\ss{}e des Fensters. Da in diesem Fall eine
Liste von Blogeinträgen dargestellt werden soll, bietet es sich an,
das Fenster in der Vertikalen grö\ss{}er zu machen als in der
Horizontalen. In diesem Fall wählen wir eine Grö\ss{}e von 360 mal 640
...
Das Hauptfenster enthält also ein Rechteck der Grö\ss{}e 360 mal 640
Pixeln und hat eine wei\ss{}e Hintergrundfarbe (\ospcmd{white}). Wir
definieren au\ss{}erdem gleich ein \ospcmd{property} namens
...
Qt-Modul per \ospcmd{import QtWebKit 1.0} au\ss{}erdem das
...
\index{QML!ListView}%
Schlie\ss{}lich wird innerhalb des Rechtecks das erste GUI-Element
...
\ospcmd{mainScreen}) erben soll. Au\ss{}erdem erhält die Liste sofort den
Fokus für Tastatureingaben und soll schlie\ss{}lich eine vertikale Liste
...
\subsection{Die Daten als Modell}
\label{sec:qmldatamodels}
...
Au\ss{}erdem soll ja ein Klick auf einen der Einträge später auch eine
Aktion auslösen. Dazu implementieren wir im nächsten Schritt unser
eigenes, aus QML-Primitiven aufgebautes GUI-Element.


\subsection{Eigene GUI-Elemente komponieren}
\label{sec:createcustomqmlelements}

Um ein \ospcmd{item}-Element des RSS anzuzeigen, werden wir nun unser
eigenes Widget implementieren. Es soll einigerma\ss{}en ansehnlich sein
...
Modells füllen. Der Code dazu sieht folgenderma\ss{}en aus:
...
hei\ss{}t, sie verweisen lediglich auf bestimmte Werte in Unterelementen,
...
\index{QML!Abstände}%
\index{QML!Anker}%
Anschlie\ss{}end definieren wir Höhe und Breite des Listen-Elements sowie
seine Unterelemente. Normalerweise verlangt jedes Element genaue
Angaben zum Layout; in QML ist das einmal die Grö\ss{}e, zum anderen die
...
Positionsangaben von seinem Elternelement. Anschlie\ss{}end kann man die
...
wobei dessen Grö\ss{}e von uns definiert wurde und der \ospcmd{ListView}
...
die grö\ss{}tmögliche Kontrolle, wenn man direkt im Code arbeitet.
...
\ospcmd{delegate}-Attributs folgenderma\ss{}en an:
...
\subsection{Signale und Zustände}
\label{sec:qmlsignalsandstates}
...
\ospcmd{anchors.fill} Grö\ss{}e und Position des Rechtecks. Die letzte
...
an das Ende des äu\ss{}eren \ospcmd{Item}-Elements mit der ID
...
Der Text wird bei Klick auf ein Listenelement nun wei\ss{} und erhält den
...
Maustaste wird au\ss{}erdem das gewählte Element mit einer anderen
Hintergrundfarbe gekennzeichnet.


\subsection{Zustandsänderungen und View-Wechsel}
\label{sec:qmlchangestateandview}
...
animieren, so dass schlie\ss{}lich zwischen Liste und Webseite eine
...
Funktionalität au\ss{}er der Anzeige der Webseite bereitstellt.  Weder
...
noch zwei Zeilen hinzu, so dass das Attribut folgenderma\ss{}en aussieht:
...
\ospcmd{WebView} als URL zugewiesen wird. Au\ss{}erdem setzen wir den
...
\section{Vordefinierte QML-Elemente}
\label{sec:qtcomponents}
...
\begin{ospdeflist}
  ...
  \ospdefitem{Hilfselemente (Beispiele: \ospcmd{Connections},
    ...
    Zugriff auf Qt\hyp Funktionen, die reiner QML- oder Javascript-Code
...
etwa für den \ospcmd{WebView} (\ospcmd{import QtWebKit
  1.0}) und Partikeleffekte (\ospcmd{import Qt.labs.particles
  1.0}) notwendig. Mit der zunehmenden Modularisierung
...
\section{QML und Python}
\label{sec:qmlandpython}

\index{QML!Integration in Python-Code}%
Bisher lie\ss{}en sich alle Beispiele einwandfrei mit dem QML Viewer
...
\subsection{QML-Views in Python-Anwendungen einbetten}
\label{sec:embedqmlinpython}
...
auch entfallen kann. Anschlie\ss{}end wird der View per \ospcmd{show()}
...
dass sich der View an die Grö\ss{}e des \emph{Root Object} anpassen soll.
...
immer genau so gro\ss{} ist, wie es die Angaben in \ospfile{main.qml} für
Höhe und Breite festlegen. Prinzipiell lässt sich die Fenstergrö\ss{}e
...
Fenstergrö\ss{}e angepasst. Das Layout der QML-Datei ist fix und in
unserem Fall immer genau 360 mal 640 Pixel gro\ss{}. Die initiale Grö\ss{}e
des Fensters richtet sich also nach der Grö\ss{}e des QML-Inhalts und
...
Damit sich der QML-Inhalt an die Grö\ss{}e des Fensters anpasst, muss der
...
Grö\ss{}e des deklarativen Views an. Wenn Sie in Listing
...
sich das dargestellte QML-Element in der Grö\ss{}e an das Fenster
anpasst.  Ändern Sie mit der Maus die Grö\ss{}e des Fensters, füllt
das Haupt-QML-Element jeweils die gesamte Grö\ss{}e des Fensters aus.
...
Scrollbalken, falls der Inhalt eines Elements grö\ss{}er als der View ist;
...
Der Modus \ospcmd{SizeViewToRootObject} erlaubt es nun, die Grö\ss{}e des
Hauptfensters bzw. des Elements zur Darstellung der QML-Datei
festzulegen, dabei aber den QML-Inhalt grö\ss{}er als das Fenster bzw. den
...
verzichten müssen. Zunächst darf man dann in der QML-Datei keine Grö\ss{}e
...
schon bekannten XML-Modells für den RSS-Feed sieht dann folgenderma\ss{}en
aus:
...
Sie keine festen Grö\ss{}enangaben zu den einzelnen Elemente machen.
...
Eintrags. Beachten Sie wieder, dass keine absoluten Grö\ss{}en mehr
...
Ähnlich lie\ss{}e sich auch eine Wechsel der Hintergrundfarbe für das
...
liegt daran, dass der \ospcmd{QDeclarativeView} noch keine Grö\ss{}e hat,
schlie\ss{}lich haben wir ja die Grö\ss{}enangabe aus der QML-Datei entfernt,
um das Layout flexibel zu machen. Der View hat in diesem Fall keine
Standardgrö\ss{}e und wird einfach nicht angezeigt. Dies lässt sich
ändern, indem wir ihm per \ospcmd{resize} eine initiale Grö\ss{}e
...
gruppiert und angezeigt, dass sich das Fenster beliebig in der Grö\ss{}e
ändern lässt, ohne das Layout zu zerstören -- was zugegebenerma\ss{}en
...
grö\ss{}er als das Fenster ist. Andernfalls lassen sich die Balken wie
...
View zu gro\ss{} wird. Dieses Verhalten unterstützt der
...
\subsection{Python-Daten in QML-Views darstellen}
\label{sec:pythondatainqmlviews}
...
folgenderma\ss{}en zur Übergabe einer Liste an den QML-View
anpassen:
...
Der Rest der Python-Anwendung bleibt erhalten, schlie\ss{}lich setzen wir
...
\ospfigure{0.5}{images/qml_context2}{Interlinearversion in einem QML-View}{fig:qml_context2}
...
Context Property setzen. Au\ss{}erdem soll dieses Mal kein
...
eine Grö\ss{}e für das Hauptfenster definieren:
...
Bei Listen mit fest vorgegebenen Grö\ss{}en können Sie auf diese Weise
...
\subsection{Python-Code aus QML aufrufen und umgekehrt}
\label{sec:signalslotspythonqml}
...
den Namen des Signals als Funktionsaufruf verwendet.  Und schlie\ss{}lich
...
am Anfang der Datei nach der Grö\ss{}e des QML-Views deklariert wird. Dazu
dient die \ospcmd{MouseArea}, die hier auf den Klick reagiert. Au\ss{}er
...
schlie\ss{}lich ist das Signal \ospcmd{messageRequested()} mit keinem Slot
verbunden.

\ospfigure{0.4}{images/qml_signalslot}{Der QML-View der Beispielanwendung}{fig:qml_signalslot}
...
\ospfigure{0.25}{images/qml_signalslot_dia}{Signal-Slot-Kommunikation zwischen Python und QML}{fig:qml_signalslot_dia}
...
Textes verwenden, au\ss{}erdem werden im Code gleich die Signale und Slots
verbunden:
