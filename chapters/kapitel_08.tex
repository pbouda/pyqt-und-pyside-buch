\chapter{Grafik- und Audioprogrammierung}
\label{chap:graphicsaudio}
...
Schlie\ss{}lich werden hier noch zahlreiche QML-Elemente für die 2D- und
...
Multimedia-Komponente, da sich die API auch einigerma\ss{}en in das
...
\section{Grafikprogrammierung mit Szenen und Views}
\label{sec:graphicsscenesandviews}
...
klickt, wird es in einem eigenen, maximierten Fenster in vergrö\ss{}erter
...
\subsection{Hauptanwendung und Hauptfensterklasse mit View und Szene}
\label{sec:graphics1_main}
...
Die Hauptfenster-Klasse hei\ss{}t hier \ospcmd{PhotoViewerWindow},
...
Signalen und Slots auf. Au\ss{}erdem setzen wir gleich Fenstergrö\ss{}e und
...
Die Fenstergrö\ss{}e wählen wir so, dass die Bilder später
untereinander in einer maximalen Grö\ss{}e von 500 mal 500 Pixeln
...
\ospcmd{QGraphicsView}. Au\ss{}erdem soll gleich eine leere grafische
...
Schlie\ss{}lich sind noch zwei Signale mit einem Slot zu verbinden.  Wenn
der Benutzer ein Verzeichnis in die Eingabezeile einträgt und
Return drückt, sollen die Fotos aus diesem Verzeichnis
geladen werden. Au\ss{}erdem soll ein Verzeichnis-Browser geöffnet werden,
...
\ospcmd{loadPhotos()} vorgestellt werden. Au\ss{}er dem Laden der Fotos in
...
Fortschrittsanzeige erscheinen. Anschlie\ss{}end setzen wir die neue Szene
...
Statusleiste gelöscht. Standardmä\ss{}ig bleibt die Nachricht so lange in
...
\ospcmd{setFileMode()} können wir dem Browser-Dialog au\ss{}erdem
...
zeigen wir den Verzeichnis-Browser schlie\ss{}lich als modalen Dialog an.
...
implementiert. Anschlie\ss{}end implementieren wir die Klasse
\ospcmd{PhotoBrowser}, die eine grafische Szene mit den Bildern aus
dem aktuellen Verzeichnis erzeugt.


\subsection{Ein Foto-Browser als grafische Szene}
\label{sec:graphicsphotobrowser}
...
und umgekehrt. In Qt hei\ss{}en diese Bilder \emph{Pixmap}; die
...
jeweils in Gro\ss{}- oder Kleinschreibung. Qt-Pixmaps können zwar
...
implementieren. Da das Laden und Verkleinern der Fotos je nach Grö\ss{}e
...
Anzeige wartet, beträgt standardmä\ss{}ig 4 Sekunden und kann per Methode
...
\ospmenu{Abbrechen}. Danach folgende Start- und Zielwert, schlie\ss{}lich
...
kein Qt-Widget ist. Schlie\ss{}lich setzen wir noch den Dialog als
...
verkleinern bei Bedarf anschlie\ss{}end das Bild auf eine maximale Grö\ss{}e
von 500 mal 500 Pixel. Au\ss{}erdem wollen wir die Pixmap und den
...
beiden Elemente (Bild oder Text) die vergrö\ss{}erte Ansicht öffnet. Die
Gruppierung sorgt dafür, dass der Doppelklick einer solchen Gruppe
zugeordnet werden kann. Au\ss{}erdem lassen sich nun alle Elemente einer
...
der Methode \ospcmd{loadPhotos()} sieht dann folgenderma\ss{}en aus:
...
Methode \ospcmd{setValue()} des Dialogs aufgerufen wird. Anschlie\ss{}end
...
Typ \ospcmd{QPointF}, also eine Koordinate aus zwei Flie\ss{}komma-Werten.
...
Flie\ss{}komma-Werte für die Koordinate entgegennimmt.

\index{QVariant}%
Rückgabe der beiden \ospcmd{add}-Methoden sind zwei von
\ospcmd{QGraphicsItem} abgeleitete Objekte, aus denen anschlie\ss{}end
...
müssen wir sie au\ss{}erdem noch per Aufruf von
...
Nach dem Ende der Schleife setzen wir schlie\ss{}lich noch den
...
Doppelklicks hei\ss{}t dieser \ospcmd{mouse\-DoubleClickEvent()} und
...
schlie\ss{}lich wurde er oben als \ospcmd{QVariant} gespeichert. Der
...
\subsection{Der Foto-Betrachter als eigener Dialog}
\label{sec:graphicsfotoviewer}
...
an. Au\ss{}erdem soll der Dialog gleich maximiert werden, so dass er den
...
Au\ss{}er dem grafischen View erzeugen wir die Pixmap als Objekt
der Klasse \ospcmd{QPixmap}. Den Pfad entnehmen wir dem Parameter
\ospcmd{filepath}. Das Layout des Dialogs enthält schlie\ss{}lich nur das
...
für die Grö\ss{}enänderung des Dialogs zu überschreiben. Schlie\ss{}lich kann
auch der Benutzer die Fenstergrö\ss{}e manuell ändern, und das enthaltene
Bild soll sich dann an die neue Fenstergrö\ss{}e anpassen. Da das
...
Änderung der Fenstergrö\ss{}e soll also die Pixmap zunächst skaliert
werden, dann soll sie in eine grafische Szene eingefügt werden, die
schlie\ss{}lich dem View zugewiesen wird. Der entsprechende Handler hei\ss{}t
\ospcmd{resizeEvent()} und ruft für die beiden Aufgaben jeweils eine
weitere Methode auf:

\begin{osplisting}{Python}{Event-Handler für Änderung der Fenstergrö\ss{}e}{code:graphics1_14}
...
Fenstergrö\ss{}e skaliert und in einem eigenen Attribut
...
\index{QPixmap!scaled()}%
\begin{osplisting}{Python}{Skalieren der Pixmap auf die aktuelle Fenstergrö\ss{}e}{code:graphics1_15}
...
Die neue Grö\ss{}e der Pixmap lesen wir hier aus Höhe und Breite des
Objekts in \ospcmd{graphicview}, das sich im Layout dynamisch an die
Fenstergrö\ss{}e anpasst. Von diesen Werten ziehen wir jeweils 5 Pixel ab,
...
Damit wird jetzt bei jeder Änderung der Fenstergrö\ss{}e die Pixmap
...
\section{Audioausgabe mit QtMultimedia}
\label{sec:qtmultimediaaudio}
...
fortgeschritten, schlie\ss{}lich standen diese bei der Entwicklung eine
...
beschrieben werden. Anschlie\ss{}end wird eine kleine Beispielanwendung
...
\subsection[Abfrage der Hardware und unterstützten Codecs]{Abfrage der Hardware und unterstützten Formate}
\label{sec:audiohwandcodecs}
...
unterstützten Codecs ausgibt, sieht folgenderma\ss{}en aus:
...
\subsection{Audioplayer zum Abspielen von WAV-Dateien}
\label{sec:audiowaveplayer}
...
Klasse \ospcmd{QAudioFormat} benötigen. Au\ss{}erdem erlaubt das Modul,
...
\ospcmd{QtCore} und \ospcmd{QtGui}. Au\ss{}erdem importieren wir das Modul
\ospcmd{wave}:
...
Die Slots zum Starten und Stoppen hei\ss{}en demnach \ospcmd{start()} und
...
Anschlie\ss{}end können alle Eigenschaften der Datei durch den Aufruf der
...
notwendig sind. Au\ss{}erdem liefert \ospcmd{getparams()} zwei weitere
...
Eingabe-/Ausgabe\-gerät zu schlie\ss{}en. Darauf kommen wir gleich zurück.
...
diese dann. Anschlie\ss{}end wird überprüft, ob der Puffer als
...
wird. Au\ss{}er dem \ospcmd{ActiveState} existieren noch die drei Zustände
