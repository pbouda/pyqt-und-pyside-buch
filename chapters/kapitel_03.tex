...
\chapter{Die Qt-Tools und Python}
\label{chap:qttools}
...
\section{Internationalisierung und Lokalisierung}
...
Übersetzungen in verschiedene Sprachen. Qt enthält au\ss{}erdem noch
...
haben standardmä\ss{}ig englische Beschriftungen; Abbildung
\ospfigref{fig:messageboxquestion2} zeigt noch einmal den
Ja-Nein-Abfragedialog. Im Unterschied dazu werden die Dateidialoge aus
...
Das liegt daran, dass Qt für solche Aufgaben auf Systemdialoge setzt, unter Windows etwa eben den betriebssystem\hyp weiten Datei-Öffnen-Dialog. Das Betriebssystem selbst
sorgt hier für die Übersetzung. Für alle anderen Dialoge, vor allem
auch für die selbst implementierten, muss sich der Entwickler aber selbst
...
\subsection{Quellcode für die Übersetzung vorbereiten}
\label{sec:preparesource}
...
umschlie\ss{}en. Die Methode \ospcmd{.tr()} ist schon im Basisobjekt
\ospcmd{QObject} implementiert und steht somit in allen davon
...
\ref{sec:erstesprojekt} sieht dann zusammengefasst folgenderma\ss{}en aus:

% what is the point of repeating this listing yet again?
...
\emph{nach} dem Aufruf von \ospcmd{tr()} durchführen, nicht vorher. Solche variablen Strings werden wir nun analog zu allen anderen übersetzen.


\subsection{Übersetzungsdateien erzeugen}
\label{sec:createts}
...
Endung \ospcmd{.ts}, die sich anschlie\ss{}end im Qt Linguist öffnen
...
\begin{ospsimplelisting}
$\$$ pylupdate4 lokalisierung.py -ts lokalisierung_de.ts
\end{ospsimplelisting}

Der entsprechende Befehl bei der Entwicklung mit PySide lautet:

\begin{ospsimplelisting}
$\$$ pyside-lupdate lokalisierung.py -ts lokalisierung_de.ts
\end{ospsimplelisting}
...
hinein. Sie enth\"alt recht übersichtliches XML, das sich im Prinzip
...
folgenderma\ss{}en auf:

\begin{ospsimplelisting}
C:\Python27\Lib\site-packages\PyQt4\bin\pylupdate4.exe
\end{ospsimplelisting}
...
Nun ist ja selten so, dass man eine Python-Anwendung nur in einer einzigen
Quellcode-Datei implementiert. Häufig lagert man einzelne oder  mehrere Klassen in separate Module aus.  In
...
\subsection{Übersetzen mit dem Qt Linguist}
\label{sec:qtlinguist}
...
starten. Die ausführbare Datei hei\ss{}t \ospfile{linguist(.exe)} oder
...
die Aufteilung der insgesamt fünf Bereiche des Hauptfensters. Von
...
wurde. Um die Überprüfung des Strings abzuschlie\ss{}en, klicken Sie auf
...
gehört. In unserem Fall wird aus \ospcmd{Hallo \{0\}!} ein \ospcmd{Hello
  \{0\}!}, denkbar wäre aber auch \ospcmd{\{0\} sei gegrü\ss{}t!}. Wenn Sie
alle Übersetzungen eingetragen bzw. überprüft haben, müssen Sie die
Datei schlie\ss{}lich speichern. Die Übersetzungen und
...
\subsection{Übersetzungen im Quellcode laden}
\label{sec:loadtranslations}
...
werden schlie\ss{}lich in der Anwendung geladen.
...
\begin{ospsimplelisting}
$\$$ lrelease lokalisierung_de.ts -qm localization_de.qm
\end{ospsimplelisting}
...
Anschlie\ss{}end muss das Objekt in der Hauptanwendung \dqo{}installiert\dqc{}
...
\ospfigure{0.6}{images/lokalisierung_deutsch}{Übersetztes Hauptfenster der Beispielanwendung}{fig:lokalisierung_deutsch}
...
Der an \ospcmd{load()} übergebene String hei\ss{}t nun aber
...
zusätzlicher Parameter übergeben werden müssen. Schlie\ss{}lich handelt es
...
stehen noch die englischen Strings, schlie\ss{}lich steht in unserer
...
\ospcmd{main()} sieht dann folgenderma\ss{}en aus:
...
\subsection{Übersetzungen und Kontext}
\label{sec:translationcontext}
...
\ospcmd{A} sieht dann folgenderma\ss{}en aus:
...
Blick aussieht. Schlie\ss{}lich erscheinen die sichtbaren Strings einer
...
\subsection{GUI der Beispielanwendung mit dem Qt Designer entwerfen}
\label{sec:qtdesignerhelloworld}
...
Die Einstellung für die \ospmenu{Bildschirmgrö\ss{}e} belassen Sie auf
\ospmenu{Vorgabe}.  Die Einstellung erlaubt Ihnen, die Grö\ss{}e des
...
Die Oberfläche des Qt Designers ist in drei gro\ss{}e Bereiche geteilt.
...
schlie\ss{}lich eine Leiste mit verschiedenen Tools zur Bearbeitung
...
wie sich das Element bei Grö\ss{}enänderung des Fensters verhält. Über den
...
\ospfigure{0.6}{images/qtdesigner_new3_windowtitle}{Ändern des Fenstertitels im Eigenschaften-Editor}{fig:qtdesigner_new3_windowtitle}
...
untergeordnet, und diesem wird schlie\ss{}lich auch das Layout zugeordnet.
...
zugewiesenen Objektnamen der hinzugefügten Widgets. Der Button hei\ss{}t
...
lassen wir alle Objektnamen beim Standard, schlie\ss{}lich arbeiten wir
...
den Wert dort ändern. Auch für das Label hei\ss{}t die Eigenschaft
...
wenigen Elemente doch etwas gro\ss{} geraten. Sie können nun einerseits
die Grö\ss{}e des Hauptfensters manuell anpassen, indem Sie im
Arbeitsbereich den Rahmen des Fensters bei gedrückter Maustaste in die
richtige Grö\ss{}e ziehen (wie bei jedem beliebigen Desktop-Fenster). Der
Qt Designer kann aber auch automatisch eine passende Grö\ss{}e berechnen,
und zwar aus den Vorgaben der Elemente wie Mindestgrö\ss{}e, Länge der
...
\ospmultimenu{Formular\sm{}Grö\ss{}e anpassen}. Die Grö\ss{}e des
...
\ospfigure{0.99}{images/qtdesigner_new5_sizeandtexts}{Das Hauptfenster mit allen Widgets und automatischer Grö\ss{}e}{fig:qtdesigner_new5_sizeandtexts}
...
\ospcmd{File} ein. Untermenüpunkte erzeugen Sie anschlie\ss{}end, indem
...
Speichern Sie nun die fertige GUI. Als Dateinamen verwenden wir \ospfile{helloworld.ui}. Sie können die gespeicherte \ospfile{.ui}-Datei einmal probehalber in einem Editor öffnen: Wie bei den Übersetzungsdateien handelt es sich um eine XML-Datei, die auch
...
\subsection{GUI in das Hauptprogramm integrieren}
\label{sec:qtdesignerintegration}
...
zur Verfügung. Die Python-Variante hei\ss{}t unter PyQt \ospfile{pyuic4},
...
\ospfile{C:\bsl{}Python27\bsl{}Lib\bsl{}site-packages\bsl{}PyQt4\bsl{}bin\bsl{}pyuic4.bat},
...
\begin{ospsimplelisting}
$\$$ pyuic4 helloworld.ui > ui_helloworld.py
\end{ospsimplelisting}
...
\begin{ospsimplelisting}
$\$$ pyuic4 -o ui_helloworld.py helloworld.ui
\end{ospsimplelisting}
...
eingebunden, um alle Widgets erzeugen zu können. Anschlie\ss{}end wird die
...
\ospcmd{Ui\_MainWindow} besitzt au\ss erdem
...
Python-Skript dazu sieht folgenderma\ss{}en aus:
...
entsprechenden Funktionen verknüpft werden. Das hei\ss{}t unter Qt, wir
...
Bei \ospcmd{objektname} handelt es sich jeweils um den
...
\subsection{Lokalisierung der Qt-Designer-GUI}
\label{sec:qtdesignerlocalization}
...
zu gebrauchen. Schlie\ss{}lich stehen alle Strings nicht mehr in der
...
Sie anschlie\ss{}end die kompilierte Übersetzungsdatei:
...
\subsection{Der Anwendung weitere Dialoge hinzufügen}
\label{sec:qtdesignerdialogs}
...
\ospfile{uic}-Tool in Python-Code umgewandelt, und schlie\ss{}lich laden
...
Liste au\ss{}erdem zwei weitere Dialogarten an, die schon Buttons
...
die Grö\ss{}e des Dialogs automatisch setzen, indem Sie
\ospmultimenu{Formular\sm{}Grö\ss{}e anpassen} auswählen. Anschlie\ss{}end
...
\begin{ospsimplelisting}
$\$$ pyuic4 hellodialog.ui > ui_hellodialog.py
\end{ospsimplelisting}
...
\ospcmd{MainWindow} hei\ss{}t, sondern \ospcmd{Dialog}. Der übrige Aufbau
...
\index{QDialog}%
Anschlie\ss{}end können Sie ein Objekt der Klasse \ospcmd{QDialog}
...
verbunden. Ein Klick auf den Button schlie\ss{}t den Dialog. Bei
...
um GUI\hyp Anwendungen handelt, auch aus weiteren Dateien wie Icons,
Konfigurationsdateien oder auch den in Kapitel \ref{sec:lokalisierung}
schon besprochenen Übersetzungsdateien. Je grö\ss{}er die Anwendung, desto
mehr solcher Dateien muss der Entwickler schlie\ss{}lich schon während der
...
Typs laden möchte. Au\ss{}erdem ist dann darauf zu achten, dass die Pfade
...
\emph{Ressourcenverwaltung} grö\ss{}tenteils in den Griff bekommen. Dabei
...
\subsection{Erstellung der Ressourcendatei}
\label{sec:createressourcefile}
...
Bilddatei, die wir dann im Hauptfenster anzeigen wollen; au\ss{}erdem die
...
\subsection{Ressourcen in einer Python-Anwendung verwenden}
\label{sec:useressources}
...
eine eigene Variante des Tools mit. Unter PyQt hei\ss{}t es
...
unser Beispiel das Tool also folgenderma\ss{}en auf, um die Datei
\ospfile{rc\_helloworld.py} zu erzeugen:

\begin{ospsimplelisting}
$\$$ pyrcc4 helloworld.qrc > rc_helloworld.py
\end{ospsimplelisting}
...
zu laden. Im Code sieht das dann folgenderma\ss{}en aus:
...
ein Objekt der Klasse \ospcmd{QPixmap}. Dieses ist schlie\ss{}lich das
...
schlie\ss{}lich auch aus einem Entwicklungsprojekt die Pakete zur
