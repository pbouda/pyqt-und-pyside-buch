...
Benutzer durch Processing-Code ersetzen muss. Der Event-Handler für
Änderungen der Cursor-Position heißt nun
...
\ospcmd{Rectangle} zu schließen.


\ospsubsubsection{Der Startknopf}{Der Startknopf}{sec:webkit3button}
...
anschließend deklarieren. Hier ist der Code für den Button:
...
Zustand wird in diesem Fall nach \ospcmd{Webview} geändert, außerdem
...
drei umschließenden Elemente für den \ospcmd{codeView} geschlossen
werden:
...
\ospsubsubsection{Der Web-View}{Der Web-View}{sec:webkit3webview}

\index{QML!WebView}%
\index{QML!HTML-Interaktion}%
\index{QML!Javascript-Interaktion}%
Weiter geht es mit dem Web-View, der schließlich den Processing-Code
ausführen soll. Der Web-View soll zunächst unsichtbar sein und die
gesamte Größe des Fensters einnehmen. Außerdem brauchen wir eine
...
entsprechende Event heißt in QML \ospcmd{PressAndHold}, wir benutzen
...
Außer Layout und \ospcmd{MouseArea} lädt der Web-View eine HTML-Datei
...
\ospsubsubsection{Zustände und Zustandsübergänge}{Zustände und Zustandsübergänge}{sec:webkit3states}
...
Zustandsübergänge mit geschweifter Klammer zu schließen. Nun fehlt nur
noch die HTML-Datei \ospfile{index.html}, die im Web-View geladen
wird.


\ospsubsubsection{Die HTML-Datei}{Die HTML-Datei}{sec:webkit3html}
...
\begin{osplisting}{Processing}{Eine erste Processing-Anwendung}{code:webkit3_qml13}
void setup()
{
  size(640, 480);
  stroke(255);
...
Wenn Sie nun auf \ospcmd{run} klicken, bewegt sich eine weiße
